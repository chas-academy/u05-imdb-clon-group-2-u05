    ...
    <div class="py-4">
        ...
        <h1 class="text-lg font-semibold ">Reviews</h1>
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <h3 class="font-bold text-yellow-500">{!! $review->title !!} - {{ $review->rating }} / 5</h3>
                <p class="text-black">{!! $review->review_text !!}</p>
                <small class="text-black">{{ $review->name }}, {{ Carbon\Carbon::parse($review->created_at)->diffForHumans()}}</small>
            </div>
            @endforeach

            @if ($reviews->count() === 0)
                <p class="text-black">There are no reviews for this title yet.</p>
            @endif
        </div>
    </div>
